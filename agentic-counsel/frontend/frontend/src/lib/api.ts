...
class ApiClient {
  ...
  getToken(): string | null {
    // If no token in memory, try to get from cookies
  ...
  async getCoachingMessages(conversationId: string): Promise<APIResponse<Message[]>> {
    const response: AxiosResponse<APIResponse<Message[]>> = await this.client.get(`/api/coaching/conversations/${conversationId}/messages`);
    return response.data;
  }
